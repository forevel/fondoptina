...
add_filter( 'hybrid_attr_loop-meta', 'hoot_theme_attr_loop_meta', 5, 2 ); // hybrid_attr_archive-header in v3.0.0 ; we use it for generic loop (archive / singular etc )
add_filter( 'hybrid_attr_loop-title', 'hoot_theme_attr_loop_title', 5, 2 ); // hybrid_attr_archive-title in v3.0.0 ; we use it for generic loop (archive / singular etc )
add_filter( 'hybrid_attr_loop-description', 'hoot_theme_attr_loop_description', 5, 2 ); // hybrid_attr_archive-description in v3.0.0 ; we use it for generic loop (archive / singular etc )
...
function hoot_theme_attr_content_block_img( $attr ) {
	$attr['class'] = ( empty( $attr['class'] ) ) ? '' : $attr['class'];
	$attr['alt'] = __( 'Content Image', 'juxter' );
	$attr['itemprop'] = 'image';
	return $attr;
}
