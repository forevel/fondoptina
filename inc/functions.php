...
function checkPass($login, $password)
{
    ...
    $result = get_table_from_db_with_query('users', $fields, $keysvalues);
    if (($result == RESULT_ERROR) || ($result == RESULT_EMPTY))
        return $result;

    $rowfetched=$result['psw'];
...
function field_validator($field_descr, $field_data, $field_type, $min_length="", $max_length="", $field_required=1)
{
    ...
    {
        error_msg("Поле $field_descr является обязательным");
        return RESULT_ERROR;
    }
    ...
    if (!$field_ok) 
    {
        error_msg("Введённый $field_descr не удовлетворяет правилам ввода");
        return RESULT_ERROR;
    }
    ...
    if ($field_ok && ($min_length > 0))
    {
        ...
        {
            error_msg("$field_descr не должен быть короче $min_length символов");
            return RESULT_ERROR;
        }
    }
    ...
    if ($field_ok && ($max_length > 0))
    {
        ...
        {
            error_msg("$field_descr не должен быть длиннее $max_length символов");
            return RESULT_ERROR;
        }
    }
    return RESULT_GOOD;
...
{
    echo "<br><script type=\"text/javascript\">alert(\"$errormsg\");</script>";
}
?>